# -*- coding: utf-8 -*-
"""
Created on Thu Apr  6 00:29:45 2023
...
def Quarter_Wave(Z_l): 
    ...
    for idx,f in enumerate(x):
        ...
        ##Beta_quarter_l = ((2*np.pi)/Lamda_f) * length_wave * np.sqrt(1-(0.01*1j))  # B*l
        #thema --> ebene Welle --> verlust durch imaginärteil --> verlust von 1% 
...
#xpos_l = np.where(S_11_load_dB == ymax_load_dB)
#xmax_load_dB = np.array(x)[xpos_l[0]]
#print('xmax_load_dB,ymax_load_dB = ', xmax_load_dB,ymax_load_dB)
print('ymax_load_dB=',ymax_load_dB)

# maximum S11_dB 
ymax_dB = np.max(np.abs(S_11_in_dB))
#xpos_S11 = np.where(S_11_in_dB == ymax_dB)
#xmax_dB = np.array(x)[xpos_S11[0]]
#print('xmax_dB and ymax_dB = ', xmax_dB,ymax_dB)
...
#w = np.linspace(-ymax_dB*0.5,-ymax_dB*0.5,1000) 
#w = np.linspace(-ymax_dB*0.5,-ymax_dB*0.5,401) 
...
#set_xlabel('Frequency in [Hz]', fontsize=6)
plt.xlabel('FREQUENCY in [GHz]', fontsize=12)
#set_ylabel('IMPEDANCE [Ohm]', fontsize=6)
plt.ylabel('IMPEDANCE IN [Ohm]', fontsize=12)
#set_title('LOAD IMPEDANCE VS FREQUENCY', fontsize=8)
#plt.title('LOAD IMPEDANCE VS FREQUENCY', fontsize=12)
plt.legend(['Real part R(Z)', 'Imaginary part I(Z)'], fontsize=10)
#plt.annotate('Impedance_max Ohm',xy=(xmax, ymax), xytext=(xmax,ymax),fontsize=10)
plt.grid()
plt.tight_layout()
plt.show()
